...
@dataclass
class EarlyStoppingTrainingArguments:
    """
    EarlyStoppingTrainingArguments is the subset of the arguments which relate to the training loop itself.
    ...
    **training a classification head**. As a result, some of the training arguments can be tuples,
    ...
    batch_size: Union[int, Tuple[int, int]] = field(default=(16, 2), repr=True) # changed from repr=False

    # num_epochs is only used to conveniently set `embedding_num_epochs` and `classifier_num_epochs`
    # which are used in practice
    num_epochs: Union[int, Tuple[int, int]] = field(default=(1, 16), repr=True) # changed from repr=False
    ...
    metric_for_best_model: Optional[Tuple[str, str]] = field(default=("embedding_loss", "loss"), repr=True)  # changed from repr=False
    greater_is_better: Tuple[bool, bool] = field(default=(False, False), repr=True)  # changed from repr=False
    ...
    def __post_init__(self) -> None:
        ...
        # Optional: normalize in case someone constructed with scalars
    ...
    @property
    def head_best_metric(self) -> Optional[str]:
        if self.metric_for_best_model is not None and len(self.metric_for_best_model) > 1:
            return self.metric_for_best_model[1]
        return None
    @property
    def body_best_metric(self) -> Optional[str]:
        if self.metric_for_best_model is not None and len(self.metric_for_best_model) > 0:
            return self.metric_for_best_model[0]
        return None
