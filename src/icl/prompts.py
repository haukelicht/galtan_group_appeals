...
#    - Do _not_ infer group characteristics that are not explicitly stated.
# 3. Respond a JSON that records 
#     1. a 1-2 sentence summary of your reasoning (starting with "The mention ..." without quoting the group mention) and
...
ATTRIBUTE_CLASSIFICATION_TASK_PROMPT_TEMPLATE = f"""\
Below, you see the text of a phrase that refers to a social group ("group mention").
...
1. Read the group mention carefully.
2. Think about whether the group mentioned or described in it uses an attribute related to {{attribute_name}} to define the group. 
   Specifically, consider whether the phrase describing characterizes the groups members' as sharing attributes related to {{attribute_name}}.
   Important:
   - Only choose "yes" if the {{attribute_name}} characteristics are stated _explicitly_ in the group mention.
   - Do _not_ infer group characteristics that are not explicitly stated.
3. Respond a JSON that records 
...
Return your response as a JSON list dictionary with fields "reasoning" (str) and "classification" (str, "yes" or "no").\
"""
# NOTE: llama-index will add the response format section automatically
