...
def zip_open(file, mode):
    """Open a ZIP file

    CAVEAT: if the zip file contains multiple files, only the first file is read (ignoring '__MACOSX/*' artifcats).
    """
    # source: https://stackoverflow.com/a/72583569
    with zipfile.ZipFile(file) as zipfin:
        for filename in zipfin.namelist():
            if filename.startswith('__MACOSX'):
                continue
            return TextIOWrapper(zipfin.open(filename, mode=mode))

def count_lines(file, mode: Literal['r', 'rb']='r'): 
    n = 0
...
# def read_instructions(file: str):
