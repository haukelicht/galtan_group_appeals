...
responses <- parse_workers_responses(
  files = c(
    file.path(data_path, "annotations", "sample_round2_emarie.jsonl"),
...
social_label_cats <- label_map[c("social group", "organizational group")]
...
# sentence level agreement (diagonal: TN and TP, off-diagonal: FN and FP)
...
comments <- read_tsv(file.path(data_path, "review", "round2_coder_comments.tsv"), col_select = 1:2)
...
# export as TSV
fp <- file.path(data_path, "review", "round2_to_be_discussed.tsv")
write_tsv(tbd, fp)
